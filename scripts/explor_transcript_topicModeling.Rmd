...
#library(textstem)                   ## stemmer and lemmatizer
...
```{r}
#import and merge all CSV files into one data frame
path = 'FRESH_17_6YG55_topicModeling.csv'
myTable = data.frame(read.csv(path))
subject_id = unlist(strsplit(path, "_"))[3]
# myCorpus <- Corpus(VectorSource(myTable$text)) # First way of creating corpus
...
dtm
as.matrix(dtm)[1:10, 1:10]                       ## show first 10 words   
dim(as.matrix(dtm))
```
...
## Document classification and perdictoin 
```{r, included=FALSE}
library("RTextTools")
library("caret")
library("glmnet")
